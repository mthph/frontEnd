...
const Inquiry = () => {
    ...
    const clickHandler = async (e) => {
        ...
        async function postEnquiry(){    
            ...
            const resp = await axios.post(process.env.REACT_APP_LINKTOBACKEND+'addEnquiry',{...contactInfo});
            console.log(resp);
            //result                    
            let data = resp.data;        
            console.log(data);
...
if((successMsg!='')) {
    return (
    <div>
        <h3 className='text-dark'> {successMsg}</h3>
    </div>
    );
}
else {
    return (
        ...
        <form className="mt-2" action="">
            ...
            <div className="form-group">
                ...
                <textarea id="remarks" type="remarks"
                placeholder="Remarks"
